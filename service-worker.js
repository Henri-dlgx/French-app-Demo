...
// Chemins relatifs à la PORTÉE du service worker, pas à la racine du domaine :
// cette démo est servie sous /French-app-Demo/, et un '/' absolu renverrait
// l'utilisateur sur github.io/ (404) au clic sur une notification.
const APP_URL       = './';
const ICON          = './icon-192.png';
// Cache suffixé « -demo » : localStorage ET Cache Storage sont cloisonnés par
// origine et non par chemin, donc la démo et la copie Pages de l'app de
// production partagent le même stockage sur henri-dlgx.github.io.
const THROTTLE_CACHE = 'wm-push-throttle-v1-demo';
...
self.addEventListener('notificationclick', event => {
  ...
  const target = event.notification.data?.url ?? new URL(APP_URL, self.registration.scope).href;
  event.waitUntil(
